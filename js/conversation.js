...
function buildConversationSelection() {
  ...
  container.innerHTML = CONVERSATION_SCENARIOS.map(sc => {
    ...
    <div class="conv-scenario-card" onclick="startConversation(${sc.id})">
      ...
      <div class="conv-card-sub">${sc.titleAr}</div>
      <div class="conv-card-meta">
        <span class="topic-tag">وحدة ${sc.ch}</span>
        <span class="topic-tag" style="background:rgba(245,158,11,0.1);color:var(--accent-gold)">${sc.difficulty}</span>
        ${bestScore > 0 ? `<span class="topic-tag" style="background:rgba(52,211,153,0.1);color:var(--accent-success)">أفضل نتيجة: ${bestScore} نقطة</span>` : ''}
      </div>
    </div>`;
  }).join('');
}
...
function startConversation(scenarioId) {
  ...
  document.getElementById('convScenarioTitle').textContent = `${sc.emoji} ${sc.title}`;
  document.getElementById('convScore').textContent = `النقاط: 0`;
...
function progressConversation() {
  ...
  if (turn.speaker === 'partner') {
    // Partner Turn
    addConvMessage('partner', turn.textDe, turn.textAr);
...
function renderUserOptions(turn) {
  ...
  area.innerHTML = `
    <div class="conv-prompt-banner">🎯 المطلوب: ${turn.prompt}</div>
      ...
      ${turn.options.map((opt, i) => `
        <div class="conv-option-card" id="convOpt-${i}" onclick="selectConvOption(${i})">
          <div>
            ...
            <div style="font-size:12px;color:var(--text-secondary);margin-top:4px">${opt.textAr}</div>
          </div>
          <span style="font-size:12px;color:var(--accent-gold)">+${opt.points} نقطة</span>
        </div>
      `).join('')}
    </div>
    <div id="convVoiceArea" style="display:none" class="conv-voice-practice-area">
      <div class="conv-mic-status" id="convMicStatus">انقر زر الميكروفون وتدرب على نطق الخيار المحدد بصوتك</div>
      <button class="mic-btn" id="convMicBtn" onclick="startConvSpeech()">🎤</button>
      <div class="conv-action-row">
        <button class="btn btn-primary" onclick="skipConvSpeech()">إرسال بدون صوت ➔</button>
      </div>
    </div>
  `;
}
...
function startConvSpeech() {
  ...
  if (status) status.textContent = 'جاري الاستماع... تحدّث الآن بالألمانية';
  ...
  startSpeechRecognition(
    (transcript) => {
      ...
      if (similarity > 0.65) {
        if (status) status.textContent = `نطق رائع! (${Math.round(similarity*100)}%)`;
        showToast('نطق ممتاز! 🎉', 'success');
        
        // Award full points
        convState.score += option.points;
        document.getElementById('convScore').textContent = `النقاط: ${convState.score}`;

        setTimeout(() => {
          advanceAfterUserSelection(option, true);
        }, 1200);
      } else {
        if (status) status.textContent = `نطق غير دقيق (${Math.round(similarity*100)}%). حاول مجدداً أو اضغط إرسال.`;
        showToast('النطق غير واضح، حاول مجدداً', 'warning');
      }
    },
    () => {
      if (btn) btn.classList.remove('recording');
      if (status) status.textContent = 'تعذر التعرف على الصوت. اضغط مجدداً أو اختر إرسال.';
    }
  );
}
...
function skipConvSpeech() {
  ...
  document.getElementById('convScore').textContent = `النقاط: ${convState.score}`;
  showToast(`تم الإرسال (+${halfPoints} نقاط)`, 'info');
...
function advanceAfterUserSelection(option, spokenSuccess) {
  addConvMessage('user', option.textDe, option.textAr, spokenSuccess);
  
  convState.currentTurnIndex++;
  
  // Clean user area temporarily
  const area = document.getElementById('userInputArea');
  if (area) area.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-muted)">الشريك يكتب...</div>';
...
function endConversation() {
  ...
  const msg = pct >= 90 ? 'مذهل! تتحدث كالألمان تماماً! 🏆' : pct >= 70 ? 'أداء رائع وقدرة ممتازة على التعبير! 👏' : 'جيد! تدرب أكثر لتحسين طلاقتك 💪';

  res.innerHTML = `
    <div class="result-score ${grade}">${pct}%</div>
    <div class="result-message">${msg}</div>
    <div class="result-details">
      النقاط المحرزة: ${convState.score} من أصل ${maxScore} نقطة ${stars}<br>
      سيناريو: ${convState.scenario.titleAr}
    </div>
    <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
      <button class="btn btn-primary" onclick="startConversation(${convState.scenario.id})">🔄 أعد المحادثة</button>
      <button class="btn btn-ghost" onclick="backToConvMenu()">← قائمة السيناريوهات</button>
    </div>
  `;
}

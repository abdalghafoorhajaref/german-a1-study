  ...
  container.innerHTML = `
    <div class="page-header">
      <div class="page-title-area">
        <h1 class="page-title">✍️ كتابة المواضيع - Aufsatz-Training</h1>
        <p class="page-subtitle">15 موضوعاً متكاملاً لتطوير مهارة الكتابة بالألمانية A1</p>
      </div>
      <button class="btn btn-ghost" onclick="showSentenceBuilderModal()">📐 جُمل مساعدة</button>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:32px">
      <div class="stat-card">
        ...
        <div class="stat-info">
          ...
          <span class="stat-label">مواضيع كتبتها</span>
        </div>
      </div>
      <div class="stat-card">
        ...
        <div class="stat-info">
          ...
          <span class="stat-label">مواضيع متاحة</span>
        </div>
      </div>
      <div class="stat-card">
        <div class="stat-icon">🎯</div>
        <div class="stat-info">
          ...
          <span class="stat-label">كلمات كتبتها</span>
        </div>
      </div>
    </div>
    ...
    <div style="display:flex;gap:12px;margin-bottom:28px;flex-wrap:wrap">
      <button class="btn btn-ghost" onclick="showConnectorsPanel()">🔗 كلمات الربط</button>
      <button class="btn btn-ghost" onclick="showSentenceBuilderModal()">📐 أنماط الجمل</button>
      <button class="btn btn-ghost" onclick="showWritingGuide()">📖 دليل الكتابة</button>
    </div>

    <div class="section-title-row" style="margin-bottom:20px">
      <h2 class="section-title">اختر موضوعاً للكتابة</h2>
      <span class="section-sub">من الأسهل للأصعب</span>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:40px">
      ${WRITING_TOPICS.map(topic => {
        ...
        <div class="chapter-card" onclick="openWritingTopic(${topic.id})" style="border-top:3px solid var(--accent-primary)">
          ...
          <div class="chapter-title-ar">${topic.titleAr}</div>
          <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px">
            <span class="topic-tag">${topic.typeAr}</span>
            <span class="topic-tag">وحدة ${topic.relatedChapter}</span>
            <span class="topic-tag">${topic.targetLength.min}-${topic.targetLength.max} كلمة</span>
          </div>
          <div style="font-size:12px;color:var(--text-muted)">${topic.description.slice(0,60)}...</div>
          ${hasSaved ? `<div style="margin-top:10px;padding:6px 10px;background:rgba(52,211,153,0.1);border-radius:6px;font-size:12px;color:var(--accent-success)">✓ كتبت ${savedWords} كلمة</div>` : ''}
        </div>`;
      }).join('')}
    </div>
    ...
    <div id="connectorsPanel" style="display:none;margin-bottom:24px">
      <div class="lesson-text-section">
        <h3 style="font-size:18px;font-weight:700;margin-bottom:20px">🔗 كلمات الربط - Konnektoren</h3>
          ...
          ${CONNECTORS.map(c => `
            <div style="padding:12px;background:rgba(255,255,255,0.03);border:var(--border-subtle);border-radius:10px">
              <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
                <span style="font-size:18px;font-weight:700;color:var(--text-german);font-family:'Inter',sans-serif">${c.de}</span>
                <span style="font-size:10px;padding:2px 8px;border-radius:20px;background:rgba(79,142,247,0.15);color:var(--accent-primary)">${c.type}</span>
              </div>
              <div style="font-size:14px;color:var(--text-arabic);margin-bottom:6px">${c.ar}</div>
              <div style="font-size:12px;color:var(--text-muted);font-family:'Inter',sans-serif;font-style:italic">${c.example}</div>
              <div style="font-size:11px;color:var(--text-muted);margin-top:4px">💡 ${c.tip}</div>
            </div>`).join('')}
        </div>
      </div>
    </div>
    ...
    <div id="sentenceBuilderPanel" style="display:none;margin-bottom:24px">
      <div class="lesson-text-section">
        <h3 style="font-size:18px;font-weight:700;margin-bottom:20px">📐 أنماط الجمل المفيدة</h3>
        ${['التعريف','المهنة','العائلة','الروتين','التفضيلات','الوصف','الصحة','الرأي','الوقت','الرسائل'].map(cat => `
          <div style="margin-bottom:20px">
            ...
            ${SENTENCE_BUILDERS.filter(s => s.category === cat).map(s => `
              <div style="padding:10px 14px;background:rgba(255,255,255,0.03);border-radius:8px;margin-bottom:6px;cursor:pointer" onclick="speakGerman('${s.example.replace(/'/g, "\\'")}')">
                <div style="font-size:15px;font-weight:600;color:var(--text-german);font-family:'Inter',sans-serif;margin-bottom:4px">${s.pattern} <button class="tts-btn" style="font-size:11px">🔊</button></div>
                <div style="font-size:13px;color:var(--text-arabic);margin-bottom:4px">${s.meaning}</div>
                <div style="font-size:12px;color:var(--text-muted);font-family:'Inter',sans-serif;font-style:italic">${s.example}</div>
              </div>`).join('')}
          </div>`).join('')}
      </div>
    </div>
...
function showWritingGuide() {
  showToast('💡 انظر إلى قسم "دليل الكتابة" في كل موضوع عند فتحه!', 'info');
}
...
function openWritingTopic(id) {
  ...
  container.innerHTML = `
    <!-- Back Button -->
    <div class="page-header">
      <button class="btn btn-ghost back-btn" onclick="buildWritingPage()">← العودة للمواضيع</button>
      <div class="page-title-area">
        <h1 class="page-title">${topic.emoji} ${topic.title}</h1>
        <p class="page-subtitle">${topic.titleAr} · ${topic.typeAr}</p>
      </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1.5fr;gap:20px;align-items:start">
      ...
      <div>
        ...
        <div style="background:rgba(79,142,247,0.08);border:1px solid rgba(79,142,247,0.2);border-radius:var(--radius-lg);padding:18px;margin-bottom:16px">
          <div style="font-size:12px;color:var(--accent-primary);font-weight:700;margin-bottom:8px">🎯 ماذا ستتعلم</div>
          <div style="font-size:14px;color:var(--text-primary);line-height:1.6">${topic.description}</div>
        </div>

        <!-- Structure Steps -->
        <div style="background:var(--bg-card);border:var(--border-subtle);border-radius:var(--radius-lg);padding:20px;margin-bottom:16px">
          <h3 style="font-size:15px;font-weight:700;margin-bottom:16px;color:var(--accent-primary)">📋 هيكل الموضوع خطوة بخطوة</h3>
          ${topic.structure.map(step => `
        ...
        <!-- Key Vocabulary -->
        <div style="background:var(--bg-card);border:var(--border-subtle);border-radius:var(--radius-lg);padding:20px;margin-bottom:16px">
          <h3 style="font-size:15px;font-weight:700;margin-bottom:14px;color:var(--accent-primary)">📚 المفردات الأساسية</h3>
          <div style="display:flex;flex-direction:column;gap:6px">
            ${topic.keyVocabulary.map(v => `
              <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:rgba(255,255,255,0.03);border-radius:8px;cursor:pointer" onclick="speakGerman('${v.de.replace(/'/g,"\\'")}')">
                ...
                <span style="font-size:13px;color:var(--text-arabic)">${v.ar}</span>
              </div>`).join('')}
          </div>
        </div>
        ...
        <!-- Useful Phrases -->
        <div style="background:var(--bg-card);border:var(--border-subtle);border-radius:var(--radius-lg);padding:20px;margin-bottom:16px">
          <h3 style="font-size:15px;font-weight:700;margin-bottom:14px;color:var(--accent-primary)">💬 عبارات مفيدة</h3>
          ${topic.usefulPhrases.map(p => `
            <div style="margin-bottom:10px;padding:10px 14px;background:rgba(255,255,255,0.03);border-radius:8px;cursor:pointer" onclick="insertToWritingArea('${p.de.replace(/'/g,"\\'")} ')">
              ...
              <div style="font-size:12px;color:var(--text-arabic)">${p.ar}</div>
              <div style="font-size:10px;color:var(--text-muted);margin-top:3px">انقر لإضافتها</div>
            </div>`).join('')}
        </div>

        <!-- Grammar Focus -->
        <div style="background:var(--bg-card);border:var(--border-subtle);border-radius:var(--radius-lg);padding:20px">
          <h3 style="font-size:15px;font-weight:700;margin-bottom:14px;color:var(--accent-primary)">📐 القواعد المهمة</h3>
          ${topic.grammarFocus.map(g => `
      ...
      <div>
        ...
        <div style="background:var(--bg-card);border:var(--border-subtle);border-radius:var(--radius-lg);padding:20px;margin-bottom:16px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
            <h3 style="font-size:16px;font-weight:700">✏️ اكتب موضوعك هنا</h3>
            <div style="display:flex;gap:8px">
              <button class="tts-btn" onclick="readWritingAloud()">🔊 اقرأ</button>
              <button class="tts-btn" onclick="clearWriting()">🗑️ مسح</button>
            </div>
          </div>

          <!-- Word bank chips -->
          <div style="margin-bottom:12px">
            <div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">انقر لإدراج كلمة:</div>
            <div style="display:flex;flex-wrap:wrap;gap:6px">
              ${topic.usefulPhrases.slice(0, 5).map(p => `
              ...
              ${CONNECTORS.slice(0, 6).map(c => `
                <span class="word-helper-chip" style="background:rgba(52,211,153,0.1);border-color:rgba(52,211,153,0.2);color:var(--accent-success)" onclick="insertToWritingArea('${c.de} ')">${c.de} (${c.ar})</span>`).join('')}
            </div>
          </div>
          ...
          <textarea
            ...
            placeholder="ابدأ الكتابة هنا بالألمانية... Schreiben Sie hier auf Deutsch..."
            dir="ltr"
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-top:12px;flex-wrap:wrap;gap:10px">
            <div style="display:flex;align-items:center;gap:16px">
              <span id="writingWordCount" style="font-size:13px;color:var(--text-secondary)">0 كلمات</span>
              <span style="font-size:12px;color:var(--text-muted)">(الهدف: ${topic.targetLength.min}-${topic.targetLength.max} كلمة)</span>
            </div>
            <div style="display:flex;gap:8px">
              <button class="btn btn-ghost" onclick="showExampleEssay()">💡 أظهر مثالاً</button>
              <button class="btn btn-primary" onclick="saveAndCheckWriting()">💾 احفظ وتحقق</button>
            </div>
          </div>
        ...
        <!-- Self Checklist -->
        <div style="background:var(--bg-card);border:var(--border-subtle);border-radius:var(--radius-lg);padding:20px;margin-bottom:16px">
          <h3 style="font-size:15px;font-weight:700;margin-bottom:14px">✅ قائمة المراجعة الذاتية</h3>
          ${topic.selfCheckList.map((item, i) => `
        ...
        <div style="background:rgba(245,158,11,0.06);border:1px solid rgba(245,158,11,0.2);border-radius:var(--radius-lg);padding:20px;margin-bottom:16px">
          <h3 style="font-size:15px;font-weight:700;margin-bottom:12px;color:var(--accent-gold)">💡 نصائح الكتابة</h3>
          ${topic.writingTips.map(tip => `
        ...
        <div id="exampleEssaySection" style="display:none;background:var(--bg-card);border:1px solid rgba(52,211,153,0.2);border-radius:var(--radius-lg);padding:20px">
          <h3 style="font-size:15px;font-weight:700;margin-bottom:12px;color:var(--accent-success)">📝 موضوع نموذجي (${topic.exampleEssay.wordCount} كلمة)</h3>
          ...
          <div style="font-size:13px;color:var(--text-arabic);line-height:1.8;white-space:pre-line;border-top:var(--border-subtle);padding-top:14px">${topic.exampleEssay.translation}</div>
          <div style="margin-top:12px;display:flex;gap:8px">
            <button class="tts-btn" onclick="speakGerman(\`${topic.exampleEssay.text.replace(/`/g, "'")}\`)">🔊 استمع للمثال</button>
            <button class="btn btn-ghost" style="font-size:12px;padding:6px 12px" onclick="useExampleAsTemplate()">📋 استخدم كقالب</button>
          </div>
        </div>
      </div>
    </div>
...
function clearWriting() {
  const ta = document.getElementById('writingTextArea');
  if (ta && confirm('هل تريد مسح النص؟')) {
    ta.value = '';
    updateMainWritingCount();
  }
}
...
function readWritingAloud() {
  ...
  if (ta && ta.value.trim()) {
    ...
    showToast('اكتب شيئاً أولاً!', 'warning');
  }
}
...
function useExampleAsTemplate() {
  const ta = document.getElementById('writingTextArea');
  if (!ta || !currentWritingTopic) return;
  if (!ta.value.trim() || confirm('سيتم استبدال نصك بالقالب. هل تريد المتابعة؟')) {
    ta.value = currentWritingTopic.exampleEssay.text;
    updateMainWritingCount();
    showToast('تم نسخ المثال. عدّله بمعلوماتك الشخصية!', 'info');
  }
}
...
function saveAndCheckWriting() {
  ...
  if (words < 5) {
    fbArea.innerHTML = `<div class="feedback-bar wrong">❌ الموضوع قصير جداً. اكتب على الأقل ${min} كلمة.</div>`;
    fbArea.style.display = 'block';
    return;
  }
  if (words >= min) positives.push(`✅ طول الموضوع مناسب: ${words} كلمات`);
  if (words < min) tips.push(`📏 اكتب أكثر - الهدف ${min} كلمة على الأقل (لديك ${words})`);
  if (!text.match(/[.!?]/)) tips.push('📌 لا تنسَ النقاط في نهاية الجمل');
  if (!text.match(/^[A-ZÄÖÜ]/)) tips.push('🔠 ابدأ بحرف كبير');
  if (!text.includes('Ich')) tips.push('👤 يبدو أن "Ich" غير موجود - تذكر الكتابة عن نفسك');
  ...
  if (foundPhrases.length > 0) positives.push(`✅ استخدمت ${foundPhrases.length} عبارات مفيدة`);

  // Check connectors
  const foundConnectors = CONNECTORS.filter(c => text.toLowerCase().includes(' ' + c.de.toLowerCase() + ' '));
  if (foundConnectors.length > 0) positives.push(`✅ استخدمت كلمات ربط: ${foundConnectors.map(c=>c.de).join(', ')}`);
  else tips.push('🔗 أضف كلمات ربط مثل: und, aber, dann, weil');
  ...
  fbArea.innerHTML = `
    <h3 style="font-size:15px;font-weight:700;margin-bottom:14px;color:${score >= 60 ? 'var(--accent-success)' : 'var(--accent-warning)'}">
      ${score >= 80 ? '🏆 ممتاز!' : score >= 60 ? '👍 جيد جداً!' : '💪 استمر في التحسين!'} (${score}%)
    </h3>
    <div style="font-size:12px;color:var(--text-muted);margin-bottom:12px">تم الحفظ ✓ | ${words} كلمة</div>
    ...
    <div style="margin-top:12px;padding:10px 14px;background:rgba(79,142,247,0.08);border-radius:8px;font-size:13px;color:var(--text-secondary)">
      💡 <strong>التالي:</strong> راجع القائمة الذاتية للتأكد من اكتمال الموضوع
    </div>`;
  fbArea.style.display = 'block';
  fbArea.scrollIntoView({ behavior: 'smooth' });
  showToast('تم الحفظ! 💾 +30 نقطة', 'success');
}

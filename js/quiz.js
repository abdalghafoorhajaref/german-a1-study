...
function buildQuizSelection() {
  ...
  container.innerHTML = CURRICULUM.map(ch => {
    ...
    <div class="quiz-chapter-card" onclick="startChapterQuiz(${ch.id})">
      ...
      <div class="quiz-card-sub">${ch.titleAr}</div>
      <div class="quiz-card-score">${best > 0 ? `أفضل نتيجة: ${best}% ${stars}` : 'لم تُختبر بعد'}</div>
    </div>`;
  }).join('');
}
...
function generateQuizQuestions(chapterId) {
  ...
  // Q1-Q3: Vocabulary MCQ (German → Arabic)
  const shuffledVocab = shuffle([...chVocab]).slice(0, 5);
  shuffledVocab.forEach(word => {
    const wrongOpts = shuffle(allVocab.filter(v => v.ar !== word.ar)).slice(0, 3).map(v => v.ar);
    const options = shuffle([word.ar, ...wrongOpts]);
    questions.push({
      type: 'mcq',
      skill: 'vocabulary',
      question: `ما معنى "<span class="de-word">${word.de}</span>" بالعربية؟`,
      options,
      answer: options.indexOf(word.ar),
      explanation: `${word.de} = ${word.ar}`,
      points: 10
    });
  });

  // Q4-Q5: Vocabulary MCQ (Arabic → German)
  ...
  shuffledVocab2.forEach(word => {
    ...
    questions.push({
      type: 'mcq',
      skill: 'vocabulary',
      question: `كيف تقول "<span style="color:var(--text-arabic)">${word.ar}</span>" بالألمانية؟`,
      options,
  ...
  ch.exercises.filter(e => e.type === 'mcq').slice(0, 2).forEach(ex => {
    questions.push({
      ...
      options: ex.options,
      answer: ex.answer,
      explanation: `الإجابة الصحيحة: ${ex.options[ex.answer]}`,
      points: 15
    });
  });
  ...
  ch.exercises.filter(e => e.type === 'fillblank').slice(0, 2).forEach(ex => {
    questions.push({
      ...
      answer: ex.answer,
      hint: ex.hint,
      explanation: `الإجابة الصحيحة: "${ex.answer}"`,
      points: 15
    });
  });
  ...
  if (ch.readingTexts && ch.readingTexts[0] && ch.readingTexts[0].questions[0]) {
    ...
    questions.push({
      type: 'mcq',
      skill: 'reading',
      question: `<div style="font-size:12px;color:var(--text-muted);margin-bottom:8px">📖 ${ch.readingTexts[0].titleAr}</div>${rq.qAr}`,
      options,
      answer: options.indexOf(rq.answer),
      explanation: `${rq.qAr} → ${rq.answerAr}`,
      points: 15
    });
  }
  ...
  if (orderEx) {
    questions.push({
      type: 'order',
      skill: 'grammar',
      question: orderEx.question,
      words: shuffle([...orderEx.words]),
      answer: orderEx.answer,
      explanation: `الجملة الصحيحة: "${orderEx.answer}"`,
      points: 20
    });
  }
...
function renderQuestion() {
  ...
  // Update header
  document.getElementById('quizProgress').textContent = `سؤال ${idx + 1} من ${total}`;
  document.getElementById('quizScore').textContent = `النقاط: ${quizState.score}`;
  ...
  const skillLabel = { vocabulary: '📚 مفردات', grammar: '📐 قواعد', reading: '📖 قراءة', listening: '🎧 استماع' };
  ...
  if (q.type === 'mcq') {
    ...
    card.innerHTML = `
      ...
      <input type="text" id="fillInput" class="fill-blank-input" placeholder="اكتب إجابتك هنا..." oninput="onFillInput()" onkeydown="if(event.key==='Enter')checkAnswer()" dir="ltr" autocomplete="off" />
      <div id="quizFeedback"></div>`;
    ...
    const renderOrder = () => {
      ...
      ).join('') || '<span style="color:var(--text-muted);font-size:13px">انقر الكلمات أدناه لترتيبها...</span>';
...
function checkAnswer() {
  ...
  if (fb) {
    fb.innerHTML = `<div class="feedback-bar ${isCorrect ? 'correct' : 'wrong'}">
      ${isCorrect ? '✅ ممتاز!' : '❌ الإجابة الصحيحة:'} ${isCorrect ? '' : `<strong>${q.explanation}</strong>`}
    </div>`;
  }
...
function showQuizResults() {
  ...
  const msg = pct >= 90 ? 'ممتاز! أداء رائع! 🏆' : pct >= 70 ? 'جيد جداً! استمر! 👏' : pct >= 50 ? 'جيد! راجع المفردات الضعيفة 📚' : 'راجع الوحدة مرة أخرى 💪';
  const chName = CURRICULUM.find(c => c.id === quizState.chapter)?.titleAr || '';

  resultsEl.innerHTML = `
    <div class="result-score ${grade}">${pct}%</div>
    <div class="result-message">${msg}</div>
    <div class="result-details">
      ${correct} إجابة صحيحة من ${quizState.questions.length} سؤال | ${quizState.score} نقطة<br>
      الوحدة: ${chName}
    </div>
    <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
      <button class="btn btn-primary" onclick="startChapterQuiz(${quizState.chapter})">🔄 أعد الاختبار</button>
      <button class="btn btn-ghost" onclick="backToQuizMenu()">← قائمة الاختبارات</button>
      <button class="btn btn-ghost" onclick="navigateTo('flashcards')">🃏 راجع بالبطاقات</button>
    </div>
    <div class="answer-review" style="margin-top:32px;text-align:right">
      <h3 style="font-size:16px;margin-bottom:16px">مراجعة الإجابات:</h3>

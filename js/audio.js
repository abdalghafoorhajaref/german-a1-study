...
function buildAudioList(filterCh = 'all') {
  ...
  if (tracks.length === 0) {
    container.innerHTML = '<div style="text-align:center;color:var(--text-muted);padding:40px">لا توجد ملفات صوتية لهذه الوحدة</div>';
    return;
  }

  container.innerHTML = tracks.map(t => {
    const chData = t.ch > 0 ? CURRICULUM[t.ch - 1] : null;
    const chLabel = chData ? `وحدة ${t.ch}: ${chData.titleAr}` : 'عام';
    return `
    <div class="audio-item" data-track-id="${t.id}" onclick="playTrack('${t.id}', '${t.file.replace(/'/g,"\\'")}', '${t.label.replace(/'/g,"\\'")}', '${chLabel}')">
      ...
      <div class="audio-item-info">
        ...
        <div class="audio-item-meta">${t.labelAr}</div>
      </div>
...
function buildAudioChapterFilter() {
  ...
  usedChapters.forEach(ch => {
    ...
    opt.value = ch;
    opt.textContent = `وحدة ${ch}: ${chData ? chData.titleAr : ''}`;
    sel.appendChild(opt);
  });
}
